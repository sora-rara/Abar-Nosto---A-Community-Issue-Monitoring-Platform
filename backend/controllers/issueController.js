...
const toggleUpvote = async (req, res) => {
    try {
        ...
        if (upvoteIndex !== -1) {
            ...
            // Send reputation change notification to owner (if not self-upvote)
            if (ownerId !== userId) {
                try {
...
const addComment = async (req, res) => {
    try {
        ...
        // Send reputation change notification for comment
        try {
            await sendReputationNotification(
                userId,
                'Reputation Awarded',
...
const requestUpdate = async (req, res) => {
    try {
        ...
        if (!adminUser) {
            // Fallback: use the first available user (e.g., the requesting user? but better to use any)
        ...
        await Notification.create({
            user: adminUser._id,   // now a valid ObjectId
            type: 'update_request',
